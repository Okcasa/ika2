...
import { Loader2, Facebook, Github, Mail, CheckCircle2, ArrowRight, ArrowLeft } from 'lucide-react';
...
export function AuthDialog({ open, onOpenChange }: AuthDialogProps) {
  ...
  const handleToggleMode = () => {
    ...
    // Small delay for the fade-out effect
    setTimeout(() => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        if (!agreeToTerms) {
          setError("Can't continue — you need to agree to the Terms of Service.");
          setLoading(false);
          return;
        }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[520px] md:max-w-[560px] border border-white/10 p-0 bg-[#0b0b0f] text-stone-100 rounded-3xl shadow-2xl overflow-hidden transition-all duration-300">
        <DialogHeader className="sr-only">
          <DialogTitle>{isSignUp ? 'Create your account' : 'Sign in to ikaLeads'}</DialogTitle>
          <DialogDescription>
        ...
        <div className="p-8 space-y-6">
          ...
          {showEmailConfirm ? (
            <div className="text-center space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-2">
                ...
                <p className="text-stone-400 text-sm">
                  We sent a confirmation link to <span className="text-stone-200 font-medium">{email}</span>
                </p>
              </div>
              ...
              <Button 
                variant="outline"
                className="w-full h-11 border-white/10 hover:bg-white/5 text-stone-300 rounded-lg"
            ...
            <>
              ...
              <div className={`text-center space-y-1 transition-all duration-200 ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
                 <h2 className="text-2xl font-bold tracking-tight text-stone-100">
                     {isSignUp ? 'Create your account' : 'Sign in to ikaLeads'}
                 </h2>
                <p className="text-stone-400 text-sm">
                    {isSignUp ? 'Join us today! Please fill in the details' : 'Welcome back! Please sign in to continue'}
                </p>
              ...
              <div className={`space-y-4 transition-all duration-200 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
              ...
              <div className={`relative transition-all duration-200 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
              ...
              <div ref={formRef} className={`transition-all duration-200 ${isTransitioning ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
                    ...
                    {isSignUp && (
                      <div className="flex items-start gap-3 pt-1">
                        <CustomCheckbox
                          ...
                          id="terms"
                        />
                        <Label htmlFor="terms" className="text-xs text-stone-400 leading-relaxed cursor-pointer select-none pt-0.5">
                          I agree to the{' '}
                          <a
                            href="/terms"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-stone-200 underline underline-offset-2 hover:text-white transition-colors"
              ...
              {/* Toggle between Sign In / Sign Up - Prominent */}
              <div className={`pt-2 pb-2 text-center border-t border-white/10 transition-all duration-200 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
                <p className="text-stone-400 text-sm">
                    {isSignUp ? 'Already have an account?' : "Don't have an account?"}
                    <button
                        type="button"
                        className="ml-1.5 font-bold text-white hover:text-stone-200 transition-colors inline-flex items-center gap-1"
